...
def draw_circle(event, x, y, flags, param):
    ...
    # 그림 그리는 중
    elif (event == cv.EVENT_MOUSEMOVE):    # 마우스 움직일 때
        if (drawing == True):               # 그림 그리는 중이라면
            if (mode == True):                                          # 직사각형 모드면
                cv.rectangle(img, (ix, iy), (x, y), (0, 255, 0), -1)   # 직사각형 그리고
            else:                                                       # 아니면
                cv.circle(img, (x, y), 5, (0, 0, 255), -1)             # 원을 그린다
        
    # 그림 그리기 끝
    elif (event ==  cv.EVENT_LBUTTONUP):                           # 마우스 좌클릭 떼면
        drawing = False                                             # 그림 그리기 비활성화
        if (mode == True):                                          # 직사각형 모드면
            cv.rectangle(img, (ix, iy), (x, y), (0, 255, 0), -1)   # 직사각형 그리고
        else:                                                       # 아니면
            cv.circle(img, (x, y), 5, (0, 0, 255), -1)             # 원 그린다

img = np.zeros((512, 512, 3), np.uint8)     # 검은 색 사진
cv.namedWindow("image")                    # 창 만들고
cv.setMouseCallback("image", draw_circle)  # 마우스 입력 받을 준비 완료

while(True):
    cv.imshow("image", img)                    # 창 띄우고
