...
# 배울 함수
# cv2.setMouseCallback() -> 마우스 입력 받기
...
def draw_circle(event, x, y, flags, param):
    if (event == cv2.EVENT_LBUTTONDBLCLK):              # 만약 입력 이벤트가 마우스 좌더블클릭이라면
        cv2.circle(img, (x, y), 100, (255, 0, 0), -1)   # 입력받은 좌표에 반지름 100, 파란색인 안이 꽉찬 원 생성
...
# 4. 마우스 버튼
# 5. 이건 모르겠다 ㅎㅎ 
cv2.setMouseCallback("image", draw_circle)  # 이 창에서 마우스 입력 받으면 draw_circle 함수 실행

while (True):
    cv2.imshow("image", img)            # 이미지 출력
    if (cv2.waitKey(20) & 0xFF == 27):  # esc 누르면 창 종료
        break
