...
# 선 그리기
# cv.line(사진, 좌표1, 좌표2, RGB색, [굵기=1, [선종류, [시프트=0]]])
# 뒤에 선종류랑 시프트는 무시하셈 나도 뭔지 모름 ㅎㅎ
img = cv.line(img, (0, 0), (511, 511), (255, 0, 0), 5)
# (0, 0)에서 (511, 511)을 잇는 색깔 (255, 0, 0) == 파란색인 5 px 짜리 굵기의 선

# 직사각형 그리기
# cv.rectangle(사진, 좌표1(좌상단), 좌표2(우하단), RGB색, [굵기=1, [선종류, [시프트=0]]])
img = cv.rectangle(img, (384, 0), (510, 128), (0, 255, 0), 3)
# (0, 255, 0) == 초록색

# 원 그리기
# cv.circle(사진, 중심좌표, 반지름, RGB색, [굵기=1, [선종류, [시프트=0]]])
img = cv.circle(img, (447, 63), 63, (0, 0, 255), -1)
# (0, 0, 255) == 빨간색
# 굵기가 -1이면 내부를 꽉 채움

# 타원 그리기
# cv.ellipse(사진, 중심좌표, 축 길이(긴반지름, 짧은반지름), 각도(반시계방향), 호 시작각도(시계방향), 호 종료각도(시계방향), RGB색, [굵기=1, [선종류, [시프트=0]]])
